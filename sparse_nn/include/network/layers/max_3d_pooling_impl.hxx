...
template<typename T>
Max3dPooling<T>::Max3dPooling(unsigned kernel_size, unsigned stride, unsigned padding): 
            BaseConvolution<T>(1, 1, kernel_size, stride, padding)
{
    // nothing here
}
template<typename T>
Max3dPooling<T>::~Max3dPooling()
{
    // nothing here
}

template<typename T>
Array<T> Max3dPooling<T>::operator()(Array<T>& input_image)
{
    ...
    Array<T> output_image(BaseConvolution<T>::n_output_rows, BaseConvolution<T>::n_output_cols);
        ...
        for(; row_thread_nb < number_strips; row_thread_nb += num_threads )
        {
            ...
            for(unsigned row = 0; row < n_output_rows_strip; ++row){
                for(unsigned column = 0; column < BaseConvolution<T>::n_output_cols; ++column){
                    output_image(row + from_output_row, column) = maxBlock(input_image_strip, row, column, bottom_padding, top_padding);
                }
            }        
        }
    }
...
template<typename T>
Array<T> Max3dPooling<T>::Forward(Array<T>& input_image)
{
    BaseConvolution<T>::n_input_channels = input_image.nchannels;
    BaseConvolution<T>::n_input_rows = input_image.nrows;
    BaseConvolution<T>::n_input_cols = input_image.ncols;
...
template<typename T>
T Max3dPooling<T>::maxBlock(Array<T>& input, unsigned row, unsigned column, unsigned bottom_padding, unsigned top_padding)
{   
    ...
    for(unsigned input_channel = 0; input_channel < BaseConvolution<T>::n_input_channels; ++input_channel){
        ...
        next_row += input.nrows;
    }
    return max_element;
}
template<typename T>
inline std::string Max3dPooling<T>::name()
{
    return "Max3dPooling";
}
template<typename T>
std::ostream& operator<<(std::ostream& out, Max3dPooling<T>& layer)
{
    out << "Max3dPooling" << static_cast<BaseConvolution<T>&>(layer);
    return out;
}