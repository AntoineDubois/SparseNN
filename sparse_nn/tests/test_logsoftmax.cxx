...
int main()
{
    ...
    LogSoftmax<double> log_softmax;
    Array<double> input_array(2, 3); input_array.fill(0.0);
    Array<double> output_array = log_softmax.Forward(input_array);

    if (notZero(output_array(0, 0) +std::log(2.0)) ) ++error;
    if (notZero(output_array(1, 0) +std::log(2.0)) ) ++error;
    if (notZero(output_array(0, 0) +output_array(1, 0) + 2 * std::log(2.0)) ) ++error;
    
    log_softmax(input_array);
    if (notZero(input_array(0, 0) +std::log(2.0)) ) ++error;
    if (notZero(input_array(1, 0) +std::log(2.0)) ) ++error;
    if (notZero(input_array(0, 0) +input_array(1, 0) + 2 * std::log(2.0))) ++error;
