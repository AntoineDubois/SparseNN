...
int main()
{
    ...
    Max2dPooling<double> max_pooling_layer(kernel_size, stride, padding);
    ...
    if ( output_image.nchannels != n_input_channels) ++error;
    if ( output_image.nrows != 8) ++error;
    if ( output_image.ncols != 3) ++error;

    if ( output_image_forward.nchannels != n_input_channels) ++error;
    ...
    Max2dPooling<double> max_pooling_layer_padding_stride(kernel_size, stride, padding);    
    Array<double> output_image_forward2 = max_pooling_layer_padding_stride.Forward(image);
    ...
    Max2dPooling<double> max_pooling_layer2(kernel_size2, stride2, padding2);
