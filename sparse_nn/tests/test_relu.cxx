...
int main()
{
    ...
    Array<double> output_array = relu.Forward(input_array);
    if (notZero(output_array(0)) ) ++error;
    if (notZero(output_array(1)) ) ++error;
    if (notZero(output_array(2)) ) ++error;
    if (notZero(output_array(3) -1.0) ) ++error;
    if (notZero(output_array(4) -1.0) ) ++error;

    relu(input_array);
    if (notZero(input_array(0)) ) ++error;
    if (notZero(input_array(1)) ) ++error;
    if (notZero(input_array(2)) ) ++error;
    if (notZero(input_array(3) -1.0) ) ++error;
    if (notZero(input_array(4) -1.0) ) ++error;
